...
// Every file that may declare a tool schema for the TEXT surface. AGENT_TOOLS
// in index.js splices these in; a new module holding tool schemas must be added
// here or the parity check silently stops covering it.
const TEXT_TOOL_SOURCES = ['index.js', 'factTools.js', 'pipelineTools.js', 'activityTools.js'];

/**
 * Tools that live ONLY on the server and deliberately have no agent.py method.
 *
 * Voice is one-brain: on_user_turn_completed raises StopResponse and llm_node
 * is a tripwire that must never run, so nothing agent-side calls an LLM. Every
 * voice reply is generated by bb-server's /session/turn, which runs the same
 * AGENT_TOOLS registry — so these tools ARE reachable in voice, through the
 * server, without an `async def` in agent.py. The remaining @function_tool
 * methods there are legacy from the two-brain era.
 *
 * Adding dead `async def` stubs purely to satisfy this check would assert a
 * capability that cannot fire and duplicate governed logic in a second
 * language. The honest contract is: declared server-side, callable on both
 * surfaces, listed here.
 */
const SERVER_ONLY_TOOLS = new Set([
  'create_pipeline_item', 'list_pipeline_items', 'get_pipeline_item', 'update_pipeline_item',
  'list_activities', 'update_activity', 'delete_activity',
]);

test('every tool documented in the prompt exists in AGENT_TOOLS (text surface)', () => {
  const sources = TEXT_TOOL_SOURCES.map((f) => readFileSync(join(here, f), 'utf-8'));
  for (const name of promptToolNames()) {
    assert.ok(
      sources.some((s) => s.includes(`name: '${name}'`)),
...
test('every tool documented in the prompt exists in the voice agent (voice surface)', () => {
  ...
  for (const name of promptToolNames()) {
    if (SERVER_ONLY_TOOLS.has(name)) continue; // reachable in voice via /session/turn
    assert.ok(
...
test('server-only tools really are declared server-side and reachable in voice', () => {
  // The exemption above is only honest if two things hold: the tool exists in
  // the text registry (which /session/turn hands the model on EVERY turn, voice
  // included), and voice genuinely has no second brain that would bypass it.
  const sources = TEXT_TOOL_SOURCES.map((f) => readFileSync(join(here, f), 'utf-8'));
  for (const name of SERVER_ONLY_TOOLS) {
    assert.ok(
      sources.some((s) => s.includes(`name: '${name}'`)),
      `'${name}' is exempted from the voice check but is not declared server-side either`
    );
  }
  const indexSource = readFileSync(join(here, 'index.js'), 'utf-8');
  assert.ok(indexSource.includes('tools: AGENT_TOOLS'), '/session/turn must still pass AGENT_TOOLS');
  const agentSource = readFileSync(join(here, '../agent/agent.py'), 'utf-8');
  assert.ok(
    agentSource.includes('raise StopResponse'),
    'voice must remain one-brain — if agent-side generation is re-armed, these tools need voice declarations'
  );
});
